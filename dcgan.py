...
LEN = 2 # LEN*batch_size大小的数据集
img_save_path = f'images-C_dcgan-latent10-{LEN}'
...
parser.add_argument('--latent_dim', type=int, default=10, help='dimensionality of the latent space default 100')
parser.add_argument('--n_classes', type=int, default=10, help='number of classes for dataset')
parser.add_argument('--img_size', type=int, default=32, help='size of each image dimension')
parser.add_argument('--channels', type=int, default=1, help='number of image channels')
parser.add_argument('--sample_interval', type=int, default=1, help='interval between image sampling default 400')
...
class Generator(nn.Module):
    ...
    # forward method
    def forward(self, input, label): # [64, 100, 1, 1], [64, 10, 1, 1]
        x = F.relu(self.deconv1_1_bn(self.deconv1_1(input))) # [64, 256, 4, 4]
        y = F.relu(self.deconv1_2_bn(self.deconv1_2(label))) # [64, 256, 4, 4]
        x = torch.cat([x, y], 1) # [64, 512, 4, 4]
        x = F.relu(self.deconv2_bn(self.deconv2(x))) # [64, 256, 8, 8]
        x = F.relu(self.deconv3_bn(self.deconv3(x))) # [64, 128, 16, 16]
        x = F.tanh(self.deconv4(x)) # [64, 1, 32, 32]
        return x
...
class Discriminator(nn.Module):
    ...
    def forward(self, input, label): # [64, 1, 32, 32], [64, 10, 32, 32]
        x = F.leaky_relu(self.conv1_1(input), 0.2) # [64, 64, 16, 16]
        y = F.leaky_relu(self.conv1_2(label), 0.2) # [64, 64, 16, 16]
        x = torch.cat([x, y], 1) # [64, 128, 16, 16]
        x = F.leaky_relu(self.conv2_bn(self.conv2(x)), 0.2) # [64, 256, 8, 8]
        x = F.leaky_relu(self.conv3_bn(self.conv3(x)), 0.2) # [64, 512, 4, 4]
        x = F.sigmoid(self.conv4(x)) # [64, 1, 1, 1]
        return x
